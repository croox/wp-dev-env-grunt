	...
	grunt.registerTask('create_release', 'sub task: used by askpush', function () {
		...
		const createRelease = (token) => {
			axios({
				...
				.then((parsedBody) => {
					[
						chalk.green('Successfully created release ') +
							chalk.cyan(parsedBody.data.id) +
							' ' +
							chalk.yellow(parsedBody.data.name),
					...
					ghReleaseAssets(
						...
						(err, assets) => {
							if (err) {
								grunt.log.writeln(
									'uploadReleaseAsset ....  what happend? Did you kill the process?'
								);
								...
								[...assets].map((asset) =>
									grunt.log.writeln(
										chalk.green('Successfully uploaded release asset ') +
											chalk.cyan(asset)
									)
								);
							}
				...
				.catch((e) => {
					grunt.log.writeln(
						'create_release ....  what happend? Did you kill the process?'
					);
					grunt.log.writeln(e);
					done.apply();
				});
		...
		if (consts.supportedHosts.includes(repoHostName)) {
					...
					axios({
						...
						.catch((e) => {
							if (e.response.status === 404) {
								...
								grunt.log.writeln(
									'Something went wrong. Not able to create release.'
								);
			...
			grunt.log.writeln(
				chalk.yellow(
					'Currently automatic creation of releases are only supported, if repository is hosted on: ' +
						consts.supportedHosts.join('||')
				)
			);
			skipTask();
		}
