...
const { isFunction, isString, find, startsWith } = require('lodash');
...
const dist = (grunt) => {
	grunt.registerTask('dist', 'Distribute a new version', function () {
		...
		const checkStatus = (status) => {
			if (
				!['release-', 'hotfix-'].reduce(
					(acc, str) => (acc ? acc : status.current.startsWith(str)),
					false
				)
			) {
			...
			if (status.staged.length === 0) {
				return new Promise((resolve, reject) => {
					prompt([
						...
						.then((answers) => (answers.proceed ? resolve() : reject()))
						.catch((e) => {
							grunt.log.writeln(
								'checkStatus ....  what happend? Did you kill the process?'
							);
							grunt.log.writeln(e);
							reject(done.apply());
						});
			...
			new Promise((resolve, reject) => {
				prompt([
					...
					},
					{
						...
						validate: (val) =>
							val.length === 0 ? 'Please provide a description!' : true,
					},
				])
					.then((distInfo) => resolve(distInfo))
					.catch((e) => {
						grunt.log.writeln(
							'getDistInfo ....  what happend? Did you kill the process?'
						);
						grunt.log.writeln(e);
						reject(done.apply());
					});
		...
		const checkChanges = (distInfo) => {
			...
			grunt.log.writeln(
				chalk.cyan(
					'To add ' +
						(nextRelease.changes.length > 0 ? 'more ' : '') +
						'changes, cancel dist-task and run: '
				) + 'grunt addchange'
			);
			grunt.log.writeln('');
			...
			return new Promise((resolve, reject) => {
				prompt([
					...
					.then((answers) => (answers.proceed ? resolve(distInfo) : reject()))
					.catch((e) => {
						grunt.log.writeln(
							'checkChanges ....  what happend? Did you kill the process?'
						);
						grunt.log.writeln(e);
						reject(done.apply());
					});
		...
		const updateChangelog = (props) => {
			const { description, newVersion } = props;

			const newRelease = new Release(newVersion, new Date(), description);
			[...nextRelease.changes].map((change) =>
				isFunction(newRelease[change.type]) && isString(change.message)
			...
			grunt.log.writeln('');
			grunt.log.writeln(
				chalk.green('Added new release ') + newVersion + chalk.green(' to CHANGELOG.md')
			);
		...
		const setOptionsUpdateConfigs = (props) => {
			const { newRelease, changelog } = props;
