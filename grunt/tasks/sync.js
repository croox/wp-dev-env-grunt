...
const { get, omit, union } = require('lodash');
...
const getSyncSource = (grunt) => {
	...
	if (version === 'test_build') {
		...
		if (!grunt.file.exists(syncSource)) grunt.warn('"' + version + '" is no valid version');
	} else {
	...
	if (isCygwin()) {
		syncSource =
			...
			syncSource
				.replace(/\\/g, '/')
				.replace(/^\S:\//g, (match) => match.toLowerCase().replace(':', ''));
	}
...
const sync = (grunt) => {
	grunt.registerTask('sync', 'sync to wp_install(s)', function (wp_installs, version) {
		...
		const promises = [...get(grunt.option('sync'), ['wp_installs'], [])].map(
			(wp_install) =>
				new Promise((resolve) => {
					try {
						const excludeArgs = [...get(wp_install, ['args'], [])]
							.filter((arg) => arg.startsWith('!'))
							.map((arg) => arg.replace('!', ''));
						const args = union(
							[...get(wp_install, ['args'], [])].filter(
								(arg) => !arg.startsWith('!')
							),
							['--archive', '--stats']
