				...
				prompt([
					...
					.then((answers) => {
						if (answers.fixed) {
							...
							grunt.log.writeln(
								chalk.red.bold('You have to fix all conflicts and commit result!')
							);
							reject(new Error('Conflicts not fixed'));
						}
					})
					.catch((e) => grunt.log.warn(e));
			}).catch(askFixed);

		simpleGit.merge(['--no-ff', this.data.commit], (err, _mergeSummary) => {
			if (get(err, ['conflicts'], []).length > 0) {
				[
					...
					...[...err.conflicts].map(
						(conflict) => '	' + conflict.file + ':' + conflict.reason
					),
