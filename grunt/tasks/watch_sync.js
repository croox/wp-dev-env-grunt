...
const watch_sync = (grunt) => {
	grunt.registerTask(
		'watch_sync',
		'Comnbination watch and sync. Watch for filechanges, build into .test_build and sync to wp_install',
		(wp_installs) => {
			if (
				undefined === wp_installs ||
				!isString(wp_installs) ||
				wp_installs.length === 0 ||
				wp_installs === 'undefined'
			) {
