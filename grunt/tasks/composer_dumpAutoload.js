	...
	grunt.registerTask('composer_dumpAutoload', 'sub task: used by build', function () {
		...
		grunt.util.spawn(
			...
			(error, result, _code) => {
				...
				grunt.file.write(
					autoload_static_path,
					grunt.file
						.read(autoload_static_path)
						.replace(
							/^(\s*?)\$loader->classMap\s*?=\s*?(ComposerStaticInit[a-z\d]+?::\$classMap);/gm,
							(match, p1, p2) =>
								[
				...
				// Print result
				[result.stdout, result.stderr].map((str) => grunt.log.writeln(chalk.green(str)));
