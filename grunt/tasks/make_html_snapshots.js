...
const make_html_snapshots = (grunt) => {
	grunt.registerTask('make_html_snapshots', '???', function (wp_installs) {
		...
		if (!wp_installs || !wp_installs.length || wp_installs.length === 0) {
			return grunt.log.writeln('No wp_install specified. Task make_html_snapshots skipped')
				.yellow;
		}
		...
		if (!wp_installs_urls.length) {
			return grunt.log.writeln(
				'Not able to find wp_installs. Task make_html_snapshots skipped'
			).yellow;
		}
		...
		if (undefined !== missingUrl) {
			return grunt.log.writeln(
				'Missing url for wp_install "' +
					missingUrl.name +
					'". Task make_html_snapshots skipped'
			).yellow;
		}

		if (wp_installs_urls.length > 1) {
			return grunt.log.writeln(
				"This task can't process multiple wp_installs. Task make_html_snapshots skipped"
			).yellow;
		}

		wp_installs_urls = [...wp_installs_urls].map(
			(entry) => entry.url.replace(/\/$/, '') // Remove trailing slash.
		);

		// Get paths from config
		const paths = [...grunt.config('make_html_snapshots').paths]
			.filter((p) => {
				if (p.startsWith('http://') || p.startsWith('https://')) {
					grunt.log.writeln('Path should be a path, not an url: ' + p);
					return false;
				}
		...
		Promise.all(promises).then(() => done.apply());
	});
};
