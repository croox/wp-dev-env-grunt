...
const startGrunt = (grunt) => {
	...
	['node_modules/wp-dev-env-grunt/grunt/hooked/*.js', 'grunt/hooked/*.js'].map((filepath) =>
		grunt.file.expand({}, [filepath]).filter((file) => require(path.resolve(file))(grunt))
	);
	...
	grunt.option(
		'pattern',
		grunt.hooks.applyFilters('startGrunt.option.pattern', {
			exclude: [
				'!*~',
				'!**/*~',
				'!#*#',
			...
			excludeFromRoot: [
				...
				'!html_snapshots',
				'!html_snapshots/**/*',
				...(pkg.projectType === 'theme' ? ['!templates', '!templates/**/*'] : []),
			],
		})
	);
	// Set option compress
	if (!isBoolean(grunt.option('compress'))) grunt.option('compress', false);
	// Set option purge
	if (!isBoolean(grunt.option('purge'))) grunt.option('purge', false);
